...
export default function ThankYou() {
  ...
  const send = useMutation({
    ...
    onSuccess: () => {
      Alert.alert('❤️ Sent', 'Your thank you is on its way.');
      router.back();
    },
  });
  ...
  return (
    <Screen>
      ...
      <T variant="title">Send a thank you ❤️</T>
      {params.name ? <T color={colors.textMuted}>To {params.name}</T> : null}
      <Row style={{ marginVertical: spacing.lg }}>
        <Chip label="✍️ Text" selected={kind === 'TEXT'} onPress={() => { setKind('TEXT'); setMediaUrl(null); }} />
        <Chip label="🎤 Voice" selected={kind === 'VOICE'} onPress={() => { setKind('VOICE'); setMediaUrl(null); }} />
        <Chip label="📷 Photo" selected={kind === 'IMAGE'} onPress={() => { setKind('IMAGE'); setMediaUrl(null); }} />
      </Row>
      ...
      <Field label="Message" multiline value={body} onChangeText={setBody} placeholder="Thank you so much! I absolutely love it ❤️" style={{ marginTop: spacing.md }} />
