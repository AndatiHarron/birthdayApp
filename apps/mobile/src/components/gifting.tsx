...
import { Avatar, Badge, Button, Card, Icon, Row, T } from './ui';
...
export function BirthdayCard({ birthday, compact }: { birthday: TrackedBirthdayDto; compact?: boolean }) {
  ...
  return (
    <Card onPress={() => router.push(`/birthday/${birthday.id}`)} style={[{ marginBottom: spacing.md }, today && { borderColor: colors.accent, backgroundColor: colors.accentSoft }]}>
      <Row gap={spacing.md}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <T variant="label" color={today ? colors.accent : colors.brand} style={{ marginTop: 2 }}>
      ...
      {!compact ? (
        <Row gap={spacing.sm} style={{ marginTop: spacing.md }}>
          ...
          <Button small variant="secondary" icon="bag" title="Send gift" onPress={() => router.push({ pathname: '/send-gift', params: { birthdayId: birthday.id } })} style={{ flex: 1 }} />
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [width === 'fill' ? { flex: 1 } : { width }, { opacity: pressed ? 0.85 : 1 }]}>
      <View style={{ borderRadius: radius.lg, overflow: 'hidden', backgroundColor: colors.surfaceMuted, aspectRatio: 1, alignItems: 'center', justifyContent: 'center' }}>
        {image ? <Image source={{ uri: image }} style={{ width: '100%', height: '100%' }} contentFit="cover" transition={150} /> : <Icon name="gift" size={40} color={colors.textFaint} />}
      </View>
    ...
    <Card onPress={onPress} style={{ marginBottom: spacing.md }}>
      <Row gap={spacing.md} style={{ alignItems: 'flex-start' }}>
        <View style={{ width: 72, height: 72, borderRadius: radius.md, overflow: 'hidden', backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' }}>
          {item.imageUrl ? <Image source={{ uri: item.imageUrl }} style={{ width: 72, height: 72 }} contentFit="cover" /> : <Icon name="gift" size={26} color={colors.textFaint} />}
        </View>
